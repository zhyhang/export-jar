...
/**
 * Dialog that hosts a {@link FileListTree} (owned {@link SelectFilesDialog.VirtualFileList} subclass)
 * without subclassing {@link SelectFilesDialog}.
 * <p>
 * Previously this class extended {@code SelectFilesDialog} and used reflection to replace its private
 * {@code myFileList} field. That approach is fragile under JBR 21 module restrictions. This rewrite
 * extends {@link DialogWrapper} directly and constructs the tree itself.
 * </p>
 */
public class FileListDialog extends DialogWrapper {
    ...
    private final FileListTree fileListTree;
    ...
    public FileListDialog(Project project, @NotNull List<? extends VirtualFile> files, @Nullable String prompt,
        ...
        super(project);
        this.fileListTree = new FileListTree(project, selectableFiles, deletableFiles, files);
        init();
        ...
        fileListTree.setInclusionListener(handler::reIncludeExcludeBySelections);
        fileListTree.addSelectionListener(handler::updateIncludeExcludeWhenSelectChange);
        fileListTree.addGroupingChangeListener(handler::groupByChanged);
        fileListTree.setCellRenderer(new FileListTreeCellRender(this, fileListTree.getCellRenderer()));
    }

    @Override
    protected @Nullable JComponent createCenterPanel() {
        // Replicate AbstractSelectFilesDialog.createCenterPanel:
        // toolbar at NORTH, scrollable tree at CENTER
        final DefaultActionGroup group = createToolbarActions();
        final ActionToolbar toolbar = ActionManager.getInstance()
                .createActionToolbar("ExportFileList", group, true);
        toolbar.setTargetComponent(fileListTree);
        final JPanel toolbarPanel = new TreeActionsToolbarPanel(toolbar, fileListTree);

        final JPanel panel = new JPanel(new BorderLayout());
        panel.add(toolbarPanel, BorderLayout.NORTH);
        panel.add(ScrollPaneFactory.createScrollPane(fileListTree), BorderLayout.CENTER);
    ...
    protected DefaultActionGroup createToolbarActions() {
        ...
        // Add the grouping separator and ChangesView.GroupBy action so the plugin's own
        // SetDirectoryNoCollapseChangesGroupingAction (registered in plugin.xml) and the
        // standard directory/module grouping options are accessible from the file-list toolbar.
        group.add(Separator.getInstance());
        final AnAction groupByAction = ActionManager.getInstance().getAction("ChangesView.GroupBy");
        if (groupByAction != null) {
            group.add(groupByAction);
        }
    ...
    /**
     * Returns the owned {@link FileListTree} (a {@link SelectFilesDialog.VirtualFileList} subclass).
     * Callers that previously used {@code SelectFilesDialog.getFileList()} continue to work unchanged.
     */
    @NotNull
    public ChangesTree getFileList() {
        return fileListTree;
    }

    /**
     * Returns the virtual files currently checked (included) in the tree.
     * Semantics match {@code SelectFilesDialog.getSelectedFiles()} which returned
     * {@code myFileList.getIncludedChanges()}.
     */
    @NotNull
    public Collection<VirtualFile> getSelectedFiles() {
        return fileListTree.getIncludedChanges();
    }

    public void addFileTreeChangeListener(BiConsumer<FileListDialog, PropertyChangeEvent> listener) {
        fileListTree.addPropertyChangeListener(e -> listener.accept(this, e));
    }
    ...
    private static class FileListTree extends SelectFilesDialog.VirtualFileList {
